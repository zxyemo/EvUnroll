...
class EdgeSmooth_loss(nn.Module):
    def __init__(self, weight=100.0, edge_aware=True):
        super(EdgeSmooth_loss, self).__init__()
        self.edge_aware = edge_aware
        self.weight = weight ** 2

    def forward(self, flow, img, rs=True):
        ...
        if rs:
            if self.edge_aware:
                ...
                return torch.mean(weight_gw * flow_gw) + 0.1 * torch.mean(weight_gh * flow_gh)
            else:
                return  torch.mean(flow_gw) + 0.1 * torch.mean(flow_gh)
        else:
            if self.edge_aware:
