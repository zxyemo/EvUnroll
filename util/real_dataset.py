...
class Sequence_Real_Video(Dataset):
    def __init__(self, cfg, seq_name):
    ...
    def __len__(self):
        return self.num_input * self.num_output
    ...
    def get_event(self, idx):
        ...
        event = self.events[start_idx:end_idx].copy()
        event[:,0] = event[:,0] - self.interval_time * idx
        #print(event.shape, event[:,0].min(), event[:,0].max())
        return event
    
    def __getitem__(self, idx):
        assert idx < self.num_input * (self.num_output )
        index = idx // (self.num_output )
        t = idx % (self.num_output )
        #print(index, t)
        if self.events is None:
            self.events = np.load(self.event_file)['event']
        ...
        timestamp = (t + 1) / (self.num_output + 1)
